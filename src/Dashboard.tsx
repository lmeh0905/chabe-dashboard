              ...
              <div style={{ background: "#fff", borderRadius: 16, boxShadow: "0 8px 40px rgba(0,0,0,0.2)", padding: 24, width: "100%", maxWidth: 420, maxHeight: "80vh", overflow: "auto" }} onClick={(e) => e.stopPropagation()}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 16 }}>
                  ...
                  {holidays.filter(h => h.date.startsWith(String(attYear))).sort((a, b) => a.date.localeCompare(b.date)).map(h => (
                    <div key={h.id} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 12px", background: editHolidayId === h.id ? "rgba(0,122,255,0.04)" : "#f9f9fb", borderRadius: 10, border: editHolidayId === h.id ? "1px solid rgba(0,122,255,0.2)" : "1px solid #f0f0f0" }}>
                      {editHolidayId === h.id ? (
                        <>
                          <input type="date" value={editHolidayDate} onChange={(e) => setEditHolidayDate(e.target.value)} style={{ width: 120, padding: "4px 6px", borderRadius: 6, border: "1px solid #e5e5ea", fontSize: 11, fontFamily: "inherit" }} />
                          <input type="text" value={editHolidayName} onChange={(e) => setEditHolidayName(e.target.value)} style={{ flex: 1, padding: "4px 8px", borderRadius: 6, border: "1px solid #e5e5ea", fontSize: 12, fontFamily: "inherit", fontWeight: 600 }} />
                          <button onClick={async () => {
                            if (!editHolidayDate || !editHolidayName.trim()) return;
                            await updatePublicHoliday(h.id, editHolidayDate, editHolidayName.trim());
                            const updated = await fetchPublicHolidays(attYear);
                            setHolidays(updated);
                            setEditHolidayId(null);
                          }} style={{ background: "#007aff", color: "#fff", border: "none", borderRadius: 6, padding: "4px 10px", fontSize: 10, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>✓</button>
                          <button onClick={() => setEditHolidayId(null)} style={{ background: "#f5f5f7", color: "#86868b", border: "none", borderRadius: 6, padding: "4px 8px", fontSize: 10, cursor: "pointer", fontFamily: "inherit" }}>✕</button>
                        </>
                      ) : (
                        <>
                          ...
                          <button onClick={() => { setEditHolidayId(h.id); setEditHolidayDate(h.date); setEditHolidayName(h.name); }} style={{ background: "rgba(0,122,255,0.08)", border: "none", borderRadius: 6, padding: "4px 8px", fontSize: 10, fontWeight: 700, color: "#007aff", cursor: "pointer", fontFamily: "inherit" }}>✎</button>
